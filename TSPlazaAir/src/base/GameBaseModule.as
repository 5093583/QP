package base
{
	...
	public class GameBaseModule extends Module implements ITCPSocketSink, IReadyGame, IDestroy
	{
		...
		public function GameBaseModule()
		{
			//			super();
			//			if(stage) {
			//				addToStage();
		...
		private function notifyWindow():void
		{
			if(stage && !stage.nativeWindow.active)
		...
		protected function OnGameFrameOption(pBuffer : ByteArray,wDataSize : int,pIClientSocket:IClientSocket) : Boolean
		{
			...
			if(CMD_GF_Option.SIZE != pBuffer.bytesAvailable)	return true;
		...
		protected function OnGameFrameScene(pBuffer : ByteArray,wDataSize : int,pIClientSocket:IClientSocket) : Boolean
		{
			...
			if(TConst.TC_DEUBG == 0)
			{
				...
				Path  = 'assets/AdminAide.swf';
				if(TConst.TC_RUTIME == 'desktop' && TPather._exist(TPather._fullPath(Path)))
				{
					if(_global.m_User.GetSelfData()) 
					{
						if( (_global.m_User.GetSelfData().dwUserRight & 0x00008000) != 0 ) 
						{ 
							OnCreateAide(); 
						}
					}
		...
		protected function onRemoveSocketSink():void
		{
			try
			{
				_global.m_Tcp.RemoveSocketSink(this as ITCPSocketSink);
			}
			catch(e:Error){};
		}
		...
		protected function SendOnLine(timeLine : Number) : void
		{
			OnDestroyOnline();
