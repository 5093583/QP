...
* Use the above macros unless you are really sure this is what you want!
*/
...
* without using multithreading you need to execute main, so the static initializers for your code are run, but
* you want to prevent the static destructors from running so it is important that main does not return.
*
* By throwing an AS3 exception and preserving the stack we can effectively pause the execution of main
*/
#define AS3_GoAsync() inline_nonreentrant_as3("import com.adobe.flascc.GoingAsync;\nthrow new GoingAsync;\n")
/* return a value via an AS3 return */
...
/* marshalls an AS3 string into a C char*. */
/* This will call malloc behind the scenes so must be free'd later */
#define AS3_MallocString(CVAR, STR) inline_as3("%0 = CModule.mallocString(" #STR ");\n": "=r"(CVAR))

/* helper to find the length of an AS3 string */
#define AS3_StringLength(CVAR, STR) inline_nonreentrant_as3("%0 = " #STR ".length;\n": "=r"(CVAR))

/* declare an AS3 variable in the current function */
...
/* Copy an AS3 local variable into a local C variable */
#define AS3_GetScalarFromVar(CVAR, AS3VAR) inline_nonreentrant_as3("%0 = " #AS3VAR ";\n" : "=r"(CVAR));

/* Copy an AS3 local variable into an AS3::local::var C++ variable */
...
/* Copy an AS3::local::var C++ variable into an AS3 local variable */
...
/**
* This function will send a metric to Monocle with a string value. The metric will 
...
/**
* This function will send a metric to Monocle with an integer value. The metric will 
* show up in the metric summary panel for the frame it was sent in.
*/
